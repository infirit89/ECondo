...
import { RegisterData, LoginData, TokenResult } from '@/types/auth';
...
export async function login(loginData: LoginData): Promise<Result> {
    ...
    try {
        const res = await normalInstance
            .post<TokenResult>('/api/account/login', loginData)
...
export async function generateAccessToken(): Promise<Result<TokenResult>> {
    try {
        const cookieStore = await cookies();

        const res = await axios.
            post<TokenResult>(`${backendApiUrl}/api/account/refresh`,
                {
